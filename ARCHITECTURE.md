...
| A `stage(FixedUpdate)` system advances by simulated time, never by frame count | `run_systems()` hands the fixed schedule to `aurora_run_fixed`, which runs it once per whole step owed; tested in `aurora-codegen`'s `fixed_stage` suite at one, four and a third of a step per frame |
| A stalled frame cannot wedge the simulation | `MAX_CATCHUP_STEPS` in `aurora-runtime`: at most 8 steps run and the rest of the debt is dropped, because chasing it makes the next frame longer still |
