...
// The builtin name registry lives in aurora-types so the TYPE CHECKER and
// codegen agree on what names exist. One list, one source of truth: a name
// missing here is what makes an unresolved call a hard error in typeck.
use aurora_types::BUILTINS;
