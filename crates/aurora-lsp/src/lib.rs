...
pub fn completions(src: &str) -> Vec<Value> {
    ...
    // Builtins come from the ONE table rather than a hand-kept copy. The copy
    // that used to live here had drifted to 80 of the 391 callable builtins, so
    // completion silently stopped offering most of the language's surface and
    // every builtin added since was missing.
    for b in aurora_abi::builtin_names() {
        push(b, 3);
    }
