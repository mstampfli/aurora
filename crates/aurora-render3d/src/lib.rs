...
mod tests {
    ...
    fn emissive_material_glows_without_lights() {
        ...
        r.set_camera(
            perspective(60f32.to_radians(), 1.0, 0.1, 100.0) * view,
        ...
        assert!(
            c[1] > 150 && c[0] < 60,
            "emissive green should glow with no lights, got {c:?}"
        );
    }
    ...
    fn point_light_illuminates_an_unlit_scene() {
        ...
        assert!(
            c[0] > 40,
            "point light should illuminate the cube face, got {c:?}"
        );
    }
    ...
    fn ssao_darkens_creases() {
        ...
        let white = r.add_material(&device, &queue, &MaterialDesc::flat([1.0, 1.0, 1.0, 1.0]));
        r.set_camera(
            perspective(55f32.to_radians(), 1.0, 0.1, 100.0)
                * look_at(Vec3::new(4.0, 5.0, 6.0), Vec3::new(0.0, 0.5, 0.0), Vec3::Y),
        ...
        assert!(
            a < b,
            "SSAO must darken (occlude) the scene: with={a} without={b}"
        );
        assert!(
            b - a > 500,
            "occlusion should be clearly present: with={a} without={b}"
        );
    }
    ...
    fn point_light_casts_a_shadow() {
        ...
        r.set_camera(
            perspective(60f32.to_radians(), 1.0, 0.1, 100.0)
                * look_at(Vec3::new(6.0, 5.0, 6.0), Vec3::new(0.0, 0.5, 0.0), Vec3::Y),
        ...
        assert!(
            a < b,
            "the point light's shadow must darken the scene: with={a} without={b}"
        );
        assert!(
            b - a > 1000,
            "the cast shadow should be clearly present: with={a} without={b}"
        );
        // ...but the point light still lights the unoccluded floor (not all dark).
        assert!(
            maxch(&with) > 100,
            "some floor must remain point-lit, max={}",
            maxch(&with)
        );
    }
    ...
    fn shadows_darken_the_scene() {
        ...
        r.set_camera(
            perspective(55f32.to_radians(), 1.0, 0.1, 200.0) * view,
        ...
        assert!(
            b - a > 400,
            "cast shadow should be clearly present: with={a} without={b}"
        );
    }
    ...
    fn msaa_renders_and_resolves() {
        ...
        assert!(
            c[0] > 32 && c[0] > c[2],
            "MSAA-resolved cube should be lit red, got {c:?}"
        );
    }
    ...
    fn metallic_surface_reflects_the_sky() {
        ...
        r.set_camera(
            perspective(50f32.to_radians(), 1.0, 0.1, 100.0)
                * look_at(Vec3::new(0.0, 0.0, 4.0), Vec3::ZERO, Vec3::Y),
        ...
        assert!(
            c[2] > c[0] + 15,
            "metal should reflect the blue sky, got {c:?}"
        );
    }
    ...
    fn skybox_fills_the_background() {
        ...
        r.set_camera(
            perspective(70f32.to_radians(), 1.0, 0.1, 100.0) * view,
        ...
        assert!(
            top[2] > top[0],
            "sky should be blue-dominant, not the red clear, got {top:?}"
        );
    ...
    fn gpu_instancing_draws_many_in_one_call() {
        ...
        r.set_camera(
            perspective(60f32.to_radians(), 1.0, 0.1, 100.0)
                * look_at(Vec3::new(0.0, 1.0, 8.0), Vec3::ZERO, Vec3::Y),
        ...
        assert!(
            px(&img, w, w / 2, h / 2)[0] > 32,
            "center instance should render"
        );
        assert!(px(&img, w, 8, h / 2)[0] > 32, "left instance should render");
        assert!(
            px(&img, w, w - 8, h / 2)[0] > 32,
            "right instance should render"
        );
    }
    ...
    fn frustum_culls_offscreen_draws() {
        ...
        assert_eq!(
            r.last_drawn(),
            1,
            "the cube behind the camera must be culled"
        );
    ...
    fn debug_lines_draw() {
        ...
        assert!(
            luma(&withln) > luma(&blank) + 100,
            "a debug line should add bright pixels"
        );
    }
    ...
    fn billboard_quad_renders_unlit() {
        ...
        r.set_camera(
            perspective(60f32.to_radians(), 1.0, 0.1, 100.0)
                * look_at(Vec3::new(0.0, 0.0, 3.0), Vec3::ZERO, Vec3::Y),
        ...
        assert!(
            c[1] > 150 && c[0] < 60,
            "emissive billboard quad should be green, got {c:?}"
        );
    }
    ...
    fn animation_samples_and_interpolates_translation() {
        ...
        assert!(
            (p1 - Vec3::new(0.0, 2.0, 0.0)).length() < 1e-4,
            "got {p1:?}"
        );
    }
    ...
    fn animation_blends_two_clips() {
        ...
        let p = m[0].transform_point3(Vec3::ZERO);
        assert!(
            (p - Vec3::new(0.0, 1.0, 0.0)).length() < 1e-4,
            "blended pose should be halfway, got {p:?}"
        );
    }
    ...
    fn gpu_skinning_applies_joint_matrix() {
        ...
        r.draw(
            ...
            red,
            Mat4::IDENTITY,
            Some(std::sync::Arc::new(vec![Mat4::IDENTITY])),
        );
        let lit = render_offscreen(&mut r, &device, &queue, w, h, [0.0, 0.0, 0.0, 1.0]);
        assert!(
            px(&lit, w, w / 2, h / 2)[0] > 60,
            "identity-skinned cube should render"
        );
        ...
        r.draw(
            ...
            red,
            Mat4::IDENTITY,
            Some(std::sync::Arc::new(vec![Mat4::from_scale(Vec3::splat(
                0.0001,
            ))])),
        );
        let collapsed = render_offscreen(&mut r, &device, &queue, w, h, [0.0, 0.0, 0.0, 1.0]);
        assert!(
            px(&collapsed, w, w / 2, h / 2)[0] < 20,
            "collapsed-skin cube should vanish"
        );
    }
    ...
    fn depth_test_occludes_far_cube() {
        ...
        r.draw(
            cube,
            near_red,
            Mat4::from_translation(Vec3::new(0.0, 0.0, 1.0)) * Mat4::from_scale(Vec3::splat(0.8)),
            None,
        );
        let img = render_offscreen(&mut r, &device, &queue, w, h, [0.0, 0.0, 0.0, 1.0]);
        ...
        assert!(
            c[0] > c[1],
            "near red cube must occlude the far green one, got {c:?}"
        );
    }
}
