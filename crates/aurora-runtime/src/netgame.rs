...
/// Hard cap on one world array: 64 chunks, which is what the completeness mask holds.
const WORLD_MAX: usize = WORLD_CHUNK * 64;
/// How many independent world arrays a game gets.
///
/// One array would force everything the host owns to be re-published whenever any of it moved:
/// MARROW's warren is ~900 entries that change a few times a round, and its horde is ~170 that
/// change every frame, so sharing an array would send the warren sixty times a second to carry
/// the horde. Separate channels, one mechanism.
const WORLD_CH: usize = 4;
...
pub struct Session {
    ...
    world_gen: [u32; WORLD_CH],
    last_sent_world: [Vec<f32>; WORLD_CH],
...
impl Session {
    ...
    fn base(sock: UdpSocket, is_server: bool, server_addr: Option<SocketAddr>) -> Session {
        ...
        Session {
            ...
            host: Player::spawn(),
            bot_count: 0,
            world: std::array::from_fn(|_| Vec::new()),
            world_gen: [0; WORLD_CH],
            last_sent_world: std::array::from_fn(|_| Vec::new()),
            world_stage_gen: [0; WORLD_CH],
            world_stage: std::array::from_fn(|_| Vec::new()),
            world_stage_got: [0; WORLD_CH],
            objects: Vec::new(),
        ...
        if self.is_server {
            ...
            if !self.dedicated {
                self.lag.record(
                    st,
                    0,
                    [self.host.s[0], self.host.s[1], self.host.s[2]],
                    r,
                    hh,
                );
            }
            ...
            for c in &self.clients {
                self.lag.record(
                    st,
                    c.id as u64,
                    [c.state.s[0], c.state.s[1], c.state.s[2]],
                    r,
                    hh,
                );
            }
            ...
            self.ids = if self.dedicated {
                ...
                std::iter::once(0u32)
                    .chain(self.clients.iter().filter(|c| !c.local).map(|c| c.id))
                    .collect()
            };
        } else {
            self.tick += dt;
            ...
            self.remotes
                .retain(|(_, r)| now.saturating_sub(r.last_seen) <= 90);
            self.ids = std::iter::once(self.my_id)
                .chain(self.remotes.iter().map(|(id, _)| *id))
                .collect();
        }
    }
    ...
    fn broadcast(&mut self) {
        ...
        // The world arrays: each sent whole (in chunks) whenever it changes, and again on every
        // keyframe so a lost chunk and a late joiner are the same non-problem.
        for ch in 0..WORLD_CH {
            if self.world[ch] == self.last_sent_world[ch] && !keyframe {
                continue;
            }
            if self.world[ch] != self.last_sent_world[ch] {
                self.world_gen[ch] = self.world_gen[ch].wrapping_add(1);
                self.last_sent_world[ch] = self.world[ch].clone();
            }
            let total = self.world[ch].len();
            if total == 0 {
                // An emptied array still has to be announced, or clients keep the last one.
                let pkt = encode_world(ch, self.world_gen[ch], 0, 0, &[]);
                for c in &self.clients {
                    let _ = self.sock.send_to(&pkt, c.addr);
                }
                continue;
            ...
            while off < total {
                ...
                let pkt = encode_world(
                    ch,
                    self.world_gen[ch],
                    total,
                    off,
                    &self.world[ch][off..end],
                );
                for c in &self.clients {
                    let _ = self.sock.send_to(&pkt, c.addr);
                }
    ...
    fn on_server_packet(&mut self, pkt: &[u8], from: SocketAddr) {
        match pkt.first().copied() {
            ...
            Some(TAG_FIRE) => {
                let Some((vt, o, d, weapon)) = decode_fire(pkt) else {
                    return;
                };
                let Some(shooter) = self.clients.iter().find(|c| c.addr == from).map(|c| c.id)
                else {
                    return;
                };
                let tick = (vt as u64).min(self.server_tick);
            ...
            Some(TAG_PROJECTILE) => {
                let Some((kind, origin, vel)) = decode_projectile(pkt) else {
                    return;
                };
                let Some(shooter) = self.clients.iter().find(|c| c.addr == from).map(|c| c.id)
                else {
    ...
    fn on_client_packet(&mut self, pkt: &[u8]) {
        ...
        if pkt.first().copied() == Some(TAG_WORLD) {
            if let Some((ch, gen, total, offset, vals)) = decode_world(pkt) {
                self.take_world_chunk(ch, gen, total, offset, &vals);
            }
            return;
        }
        ...
        let Some((your_id, acked, tick, stick, players)) = decode_snapshot(pkt) else {
            return;
        };
        self.got_snap = true;
                ...
                let slot = match self.remotes.iter_mut().find(|(rid, _)| *rid == id) {
                    ...
                    None => {
                        self.remotes.push((
                            id,
                            Remote {
                                interp: InterpBuffer::new(INTERP_DELAY),
                                last: st,
                                last_seen: stick,
                            },
                        ));
    ...
    /// HOST: set the length of world array `ch` (entries beyond the old length are 0).
    pub fn set_world_len(&mut self, ch: usize, n: usize) {
        if !self.is_server || ch >= WORLD_CH {
            return;
        }
        self.world[ch].resize(n.min(WORLD_MAX), 0.0);
    }
    ...
    pub fn set_world(&mut self, ch: usize, i: usize, v: f64) {
        if !self.is_server || ch >= WORLD_CH {
            return;
        }
        if let Some(slot) = self.world[ch].get_mut(i) {
            *slot = v as f32;
        }
    }
    /// Length of world array `ch` as this peer knows it. On a client this is 0 until a COMPLETE
    /// generation has arrived, so a game can use it as the "do I have this yet?" question without
    /// a second flag.
    pub fn world_len(&self, ch: usize) -> usize {
        self.world.get(ch).map(|w| w.len()).unwrap_or(0)
    }
    /// Read one entry; 0 outside the array.
    pub fn world(&self, ch: usize, i: usize) -> f64 {
        self.world
            .get(ch)
            .and_then(|w| w.get(i))
            .copied()
    ...
    /// How many complete generations of `ch` this peer has adopted. A client watches this to
    /// notice "it changed" without diffing the array itself.
    pub fn world_gen(&self, ch: usize) -> u32 {
        self.world_gen.get(ch).copied().unwrap_or(0)
    }

    /// CLIENT: fold one chunk into the generation being assembled, publishing it once whole.
    fn take_world_chunk(&mut self, ch: usize, gen: u32, total: usize, offset: usize, vals: &[f32]) {
        if ch >= WORLD_CH {
            return;
        }
        if gen != self.world_stage_gen[ch] || self.world_stage[ch].len() != total {
            self.world_stage_gen[ch] = gen;
            self.world_stage[ch] = vec![0.0; total];
            self.world_stage_got[ch] = 0;
        }
        let end = (offset + vals.len()).min(total);
        self.world_stage[ch][offset..end].copy_from_slice(&vals[..end - offset]);
        let chunk = offset / WORLD_CHUNK;
        if chunk < 64 {
            self.world_stage_got[ch] |= 1u64 << chunk;
        }
        ...
        if self.world_stage_got[ch] == full {
            self.world[ch] = self.world_stage[ch].clone();
            self.world_gen[ch] = gen;
        }
    }
    ...
    pub fn server_projectile_origin(&self, i: usize, axis: usize) -> f64 {
        self.server_projectiles
            .get(i)
            .map(|p| p.origin[axis.min(2)] as f64)
            .unwrap_or(0.0)
    }
    pub fn server_projectile_vel(&self, i: usize, axis: usize) -> f64 {
        self.server_projectiles
            .get(i)
            .map(|p| p.vel[axis.min(2)] as f64)
            .unwrap_or(0.0)
    }
...
fn encode_world(ch: usize, gen: u32, total: usize, offset: usize, vals: &[f32]) -> Vec<u8> {
    let mut b = Vec::with_capacity(12 + vals.len() * 4);
    b.push(TAG_WORLD);
    b.push(ch as u8);
    put_u32(&mut b, gen);
...
/// `(channel, generation, total length, offset, values)` as [`encode_world`] wrote it.
type DecodedWorld = (usize, u32, usize, usize, Vec<f32>);

fn decode_world(b: &[u8]) -> Option<DecodedWorld> {
    if b.len() < 12 || b[0] != TAG_WORLD {
        return None;
    }
    let ch = b[1] as usize;
    let gen = rd_u32(b, 2);
    let total = u16::from_be_bytes([b[6], b[7]]) as usize;
    let offset = u16::from_be_bytes([b[8], b[9]]) as usize;
    let count = u16::from_be_bytes([b[10], b[11]]) as usize;
    if ch >= WORLD_CH || total > WORLD_MAX || offset + count > total || 12 + count * 4 > b.len() {
        return None;
    }
    let vals = (0..count).map(|i| rd_f32(b, 12 + i * 4)).collect();
...
#[no_mangle]
pub unsafe extern "C" fn aurora_net_sim(
    sim_fn: *const u8,
    sim_env: *const u8,
    state_len: i64,
    input_len: i64,
...
pub extern "C" fn aurora_net_set_player_meta(id: i64, slot: i64, v: f64) {
    with((), |s| {
        s.set_player_meta(id.max(0) as u32, slot.max(0) as usize, v)
    })
}
...
#[no_mangle]
pub extern "C" fn aurora_net_set_world_len(ch: i64, n: i64) {
    with((), |s| {
        s.set_world_len(ch.max(0) as usize, n.max(0) as usize)
    })
}
/// HOST: write one entry of the world-state array.
#[no_mangle]
pub extern "C" fn aurora_net_set_world(ch: i64, i: i64, v: f64) {
    with((), |s| {
        s.set_world(ch.max(0) as usize, i.max(0) as usize, v)
    })
}
...
#[no_mangle]
pub extern "C" fn aurora_net_world_len(ch: i64) -> i64 {
    read(0, |s| s.world_len(ch.max(0) as usize) as i64)
}
/// Read one entry of the world-state array; 0 outside it.
#[no_mangle]
pub extern "C" fn aurora_net_world(ch: i64, i: i64) -> f64 {
    read(0.0, |s| s.world(ch.max(0) as usize, i.max(0) as usize))
}
/// How many complete world versions this peer has adopted - watch it to notice a change without
/// diffing the array.
#[no_mangle]
pub extern "C" fn aurora_net_world_gen(ch: i64) -> i64 {
    read(0, |s| s.world_gen(ch.max(0) as usize) as i64)
}
...
pub extern "C" fn aurora_net_set_bot_meta(i: i64, slot: i64, v: f64) {
    with((), |s| {
        s.set_bot_meta(i.max(0) as usize, slot.max(0) as usize, v)
    })
}
...
#[allow(clippy::too_many_arguments)]
#[no_mangle]
pub extern "C" fn aurora_net_fire(
    ox: f64,
    oy: f64,
    oz: f64,
    dx: f64,
...
#[no_mangle]
pub extern "C" fn aurora_net_projectile_intent(
    kind: i64,
    ox: f64,
    oy: f64,
    oz: f64,
...
pub extern "C" fn aurora_net_push_kill(killer: i64, victim: i64) {
    with((), |s| {
        s.push_kill(killer.max(0) as u32, victim.max(0) as u32)
    });
}
#[no_mangle]
...
pub extern "C" fn aurora_net_push_shot(
    ...
    with((), |s| {
        s.push_shot(shooter.max(0) as u32, ox, oy, oz, ex, ey, ez, weapon)
    });
}
#[no_mangle]
...
pub extern "C" fn aurora_net_shot_field(i: i64, field: i64) -> f64 {
    read(0.0, |s| {
        s.shot_field(i.max(0) as usize, field.max(0) as usize)
    })
}
#[no_mangle]
...
pub extern "C" fn aurora_net_push_boom(source: i64, x: f64, y: f64, z: f64, intensity: f64) {
    with((), |s| {
        s.push_boom(source.max(0) as u32, x, y, z, intensity)
    });
}
#[no_mangle]
...
pub extern "C" fn aurora_net_boom_field(i: i64, field: i64) -> f64 {
    read(0.0, |s| {
        s.boom_field(i.max(0) as usize, field.max(0) as usize)
    })
}
#[no_mangle]
...
mod tests {
    ...
    fn prediction_reconciles_with_a_registered_sim() {
        ...
        assert!(
            (client.px(cid) - sx).abs() < 0.5,
            "client {} converges to server {sx}",
            client.px(cid)
        );
    }
...
mod meta_replication_test {
    ...
    fn the_world_array_replicates_whole() {
        ...
        host.set_world_len(0, n);
        for i in 0..n {
            host.set_world(0, i, (i * 3 + 1) as f64);
        }
        // A second channel, changing on its own: channels must not share a generation or a
        // staging buffer, or a horde updating every frame would re-send the warren with it.
        host.set_world_len(1, 3);
        for i in 0..3 {
            host.set_world(1, i, (900 + i) as f64);
        }
        let input = [0.0f32; 4];
        for _ in 0..60 {
        ...
        assert_eq!(client.world_len(0), n, "the client never adopted the world");
        for i in 0..n {
            assert_eq!(
                client.world(0, i),
                (i * 3 + 1) as f64,
        ...
        assert_eq!(client.world_len(1), 3, "channel 1 never arrived");
        assert_eq!(client.world(1, 2), 902.0);

        // A CHANGED world must replace it wholesale, not patch it - and touching channel 0 must
        // leave channel 1's generation alone, which is the whole point of separate channels.
        for i in 0..n {
            host.set_world(0, i, (i * 7 + 2) as f64);
        }
        let gen_before = client.world_gen(0);
        let other_gen_before = client.world_gen(1);
        for _ in 0..60 {
            client.send_input(&input);
            host.send_input(&input);
            client.update(0.016);
        ...
        assert!(
            client.world_gen(0) != gen_before,
            "the client did not notice a new world version"
        );
        assert_eq!(
            client.world_gen(1),
            other_gen_before,
            "changing channel 0 bumped channel 1"
        );
        for i in 0..n {
            assert_eq!(client.world(0, i), (i * 7 + 2) as f64, "entry {i} is stale");
        }
        ...
        let old: Vec<f64> = (0..n).map(|i| client.world(0, i)).collect();
        client.take_world_chunk(
            0,
            client.world_gen(0).wrapping_add(9),
            n,
            0,
            &[42.0; WORLD_CHUNK],
        );
    ...
    fn a_late_joiner_receives_the_world() {
        let mut host = Session::host(0).expect("host bind");
        let host_addr = host.local_addr();
        host.set_world_len(0, 5);
        for i in 0..5 {
            host.set_world(0, i, (100 + i) as f64);
        }
        let input = [0.0f32; 4];
        ...
        assert_eq!(client.world_len(0), 5, "a late joiner never got the world");
        assert_eq!(client.world(0, 3), 103.0);
    }
    ...
    fn metadata_replicates_host_and_client() {
        ...
        let read_name = |s: &Session, id: u32| -> String {
            (0..s.name_len(id))
                .map(|i| s.name_char(id, i as usize) as u8 as char)
                .collect()
        };
        assert_eq!(
            read_name(&client, 0),
    ...
    fn reconcile_does_not_inflate_position() {
        ...
        assert!(
            cy < -10.0,
            "client should have fallen well below 0 after 25 steps, got {cy}"
        );
    }
    ...
    fn reconcile_with_physics_does_not_fly() {
        ...
        extern "C" fn phys_fall_sim(_env: i64, state_ptr: i64, input_ptr: i64) {
            ...
            let grounded =
                aurora_phys3d_raycast_world(player, px, py, pz, 0.0, -1.0, 0.0, 1.2) >= 0;
            let mut vy = s[5];
            let jump = inp[3] > 0.5;
        ...
        for f in 0..120 {
            ...
            if f % 15 == 0 {
                let cid = client.my_id();
                eprintln!(
                    "frame {f}: client_pred_y={:.2} host_auth_y={:.2}",
                    client.py(cid),
                    host.py(cid)
                );
            }
        ...
        assert!(
            cy < 1.5 && cy > 0.4,
            "client should rest on the floor (~0.9), got {cy}"
        );
    }
    ...
    fn lobby_full_rejects_extra_client() {
        ...
        assert!(
            c2.rejected(),
            "the second client should be rejected (lobby full)"
        );
    }
    ...
    fn bots_replicate_as_players() {
        ...
        assert!(
            (client.px(b0) - 33.0).abs() < 0.5,
            "client saw bot0 x = {}",
            client.px(b0)
        );
        assert!(
            (client.px(b1) - 44.0).abs() < 0.5,
            "client saw bot1 x = {}",
            client.px(b1)
        );
        assert!(
            (client.meta(b0, 0) - 88.0).abs() < 0.01,
            "client saw bot0 hp = {}",
            client.meta(b0, 0)
        );
        assert!(
            (client.meta(b1, 0) - 70.0).abs() < 0.01,
            "client saw bot1 hp = {}",
            client.meta(b1, 0)
        );
        let read_name = |s: &Session, id: u32| -> String {
        ...
        assert_eq!(
            read_name(&client, b0),
            "BOT-A",
            "client should read bot0's name"
        );
        // And the client lists them among its players (self + host + 2 bots).
        let ids: Vec<i64> = (0..client.player_count())
            .map(|i| client.player_id_at(i))
            .collect();
        assert!(
            ids.contains(&(b0 as i64)) && ids.contains(&(b1 as i64)),
            "bots in player list: {ids:?}"
        );
    }
    ...
    fn host_applies_validated_client_shot() {
        ...
        assert_eq!(
            host.server_hit_victim(0),
            BOT_ID_BASE as i64,
            "victim should be the bot"
        );
        let cid = client.my_id();
        assert_eq!(
            host.server_hit_shooter(0),
            cid as i64,
            "shooter should be the client"
        );
        ...
        assert_eq!(
            client.hit_player(),
            BOT_ID_BASE as i64,
            "client should confirm its hitmarker"
        );
    }
    ...
    fn objects_replicate_to_client() {
        ...
        assert!(
            (client.object_pos(1, 0) - 11.0).abs() < 0.01,
            "crate 1 x = {}",
            client.object_pos(1, 0)
        );
        // an unrotated crate reports identity (qw = 1) so it renders upright by default
        assert!(
            (client.object_rot(1, 3) - 1.0).abs() < 0.01,
            "default crate qw = {}",
            client.object_rot(1, 3)
        );
        ...
        assert!(
            (client.object_pos(2, 0) - 18.0).abs() < 0.01,
            "moved crate 2 x = {}",
            client.object_pos(2, 0)
        );
        assert!(
            (client.object_pos(2, 1) - 1.5).abs() < 0.01,
            "moved crate 2 y = {}",
            client.object_pos(2, 1)
        );
        assert!(
            (client.object_rot(2, 1) - 0.70710677).abs() < 0.01,
            "tumbled crate 2 qy = {}",
            client.object_rot(2, 1)
        );
        assert!(
            (client.object_rot(2, 3) - 0.70710677).abs() < 0.01,
            "tumbled crate 2 qw = {}",
            client.object_rot(2, 3)
        );
        // The flung crate's VELOCITY replicates too, so the client drives its body at the host's speed.
        assert!(
            (client.object_vel(2, 0) - 4.0).abs() < 0.01,
            "crate 2 vx = {}",
            client.object_vel(2, 0)
        );
        assert!(
            (client.object_vel(2, 1) - (-2.0)).abs() < 0.01,
            "crate 2 vy = {}",
            client.object_vel(2, 1)
        );
        assert!(
            (client.object_vel(2, 2) - 1.5).abs() < 0.01,
            "crate 2 vz = {}",
            client.object_vel(2, 2)
        );
    }
    ...
    fn bots_do_not_inflate_player_count() {
        ...
        assert_eq!(
            host.player_count(),
            2,
            "host should count itself + 1 client, NOT the 3 bots"
        );
    }
    ...
    fn shots_reach_client() {
        ...
        assert_eq!(
            host.shot_count(),
            1,
            "host should see its own queued shot for local render"
        );
        host.flush_shots();
    ...
    fn projectile_intent_queues_on_host() {
        ...
        assert!(
            host.server_projectile_count() >= 1,
            "host queued no projectile"
        );
        let cid = client.my_id();
        assert_eq!(host.server_projectile_shooter(0), cid as i64, "shooter id");
        assert_eq!(host.server_projectile_kind(0), 0, "kind = rocket");
        assert!(
            (host.server_projectile_origin(0, 0) - 2.0).abs() < 0.01,
            "origin x"
        );
        assert!(
            (host.server_projectile_vel(0, 0) - 1.0).abs() < 0.01,
            "vel x"
        );
    }
    ...
    fn host_owns_client_hp() {
        ...
        assert!(
            (client.meta(client.my_id(), 0) - 37.0).abs() < 0.01,
            "client hp = {}",
            client.meta(client.my_id(), 0)
        );
    }
