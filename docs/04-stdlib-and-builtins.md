...
| `r3d_draw_tint(h, px,py,pz, yaw,pitch,roll, scale, r,g,b)` | the same, with the albedo shifted | an ADDITIVE offset, so negative components darken and `0,0,0` is exactly `r3d_draw`. Per draw rather than per model, so the same handle can be drawn light and dark in one frame - a game gets damage flashes and enemy variants from it without a second copy of the mesh |
| `r3d_draw_flash(h, x, y, z, yaw, pitch, roll, scale, r, g, b)` | draw a model with rgb added as emissive LIGHT, after the texture | `r3d_draw_tint` shifts the base colour and the texture multiplies afterwards, so on a dark surface it scales an already-small number and cannot brighten anything: measured on black armour whose own pixels average 14.9 of 255, driving that tint all the way to -1.0 moved the picture by 2.13. There is no value of it that reads. This adds light instead, which is the mechanism a hit flash or a heat glow needs, and it is the same one the energy shield already used without the Fresnel |
...
| `r3d_joint_pos(h, joint, axis) -> f64` | where a joint is in the MODEL's own space | 0.0 for a stale handle or a joint the model does not have. For a world position use `r3d_joint_world` rather than applying the host transform yourself |
| `r3d_joint_world(h, joint, axis, x, y, z, yaw, scale) -> f64` | where a joint is IN THE WORLD, for a host drawn at that placement | `r3d_joint_pos` answers in the model's own space, so every caller wanting a world position was applying the host scale, rotation and translation itself - re-deriving the composition the renderer already does in `draw_on_joint`. The placement is a parameter rather than remembered state, because a joint's world position is only meaningful against one |
| `r3d_yaw_rotate(yaw, lx, lz, axis) -> f64` | turn a LOCAL offset by a yaw into world space in the renderer's handedness (`axis` 0 = x, 2 = z) - the rotation `r3d_draw` applies, on its own. Without it a caller placing anything relative to a turned object writes `x*cos + z*sin` / `-x*sin + z*cos` by hand, and the sign of the second row looks obviously right in both directions |
| `r3d_joint_basis(h, joint, axis, comp, yaw) -> f64` | one column of a joint's world rotation basis: which way it FACES | `axis` picks the column (0/1/2), `comp` the component. Normalised. Lets a caller compute the DIRECTION a socketed prop points - rotate the prop's own axis by the socket and read the world vector - instead of judging a grip from a render. A picture that looks plausible is how a wrong weapon grip survives |
| `r3d_clip_joint_world(h, clip, t, joint, axis, x, y, z, yaw, scale) -> f64` | where a joint is IN THE WORLD when a CLIP is sampled at a TIME | The asset-side twin of `r3d_joint_world`, and the difference is a layering one. `r3d_joint_world` answers about the pose ON SCREEN, so a game rule using it to decide something is asking the animator what is true - and gets the rest pose for any body whose clip failed to load. This is a pure function of authored data, so a simulation that already owns which move is out and how far through it can place a hitbox on the weapon without asking the presentation layer anything. Only the chain from the root to `joint` is resolved, so the cost is that chain's depth and not the skeleton's size |
| `r3d_clip_joint_basis(h, clip, t, joint, axis, comp, yaw) -> f64` | which way a joint POINTS on one frame of one clip | The direction half of the pair. A blade is a segment, so a hitbox on a weapon needs where the fist is AND which way the blade leaves it; with only the position, a hitbox can only ever be a sphere at the hand |
| `r3d_clip_joint_ok(h, clip, joint) -> i64` | can this rig answer about that joint in that clip at all? | The two above fall back to the origin, which is a PLAUSIBLE answer and therefore the dangerous kind: a hitbox built on a missing clip sits at the body's feet and reads as a weapon that misses rather than as a broken build. Ask this once when a rig loads instead of testing a magic number every tick |
...
| `r3d_anim_play(h, clip, looping, speed, fade)` | ensure this clip is the one playing | IDEMPOTENT: calling it every frame with what is already running only updates `speed`, it does not re-seed the clock. That is what lets a frame loop state what should be on screen. It used to restart unconditionally, so every caller had to hand-roll a guard and the ones that forgot froze the character on frame 0 - a body that jitters and never completes a transition. `fade` crossfades from the current clip over that many seconds (0 = snap) |
| `r3d_anim_restart(h, clip, looping, speed, fade)` | play from the top even if already playing | the explicit "again", for the combo step that reuses one clip. Rare by design: if you are calling this from a frame loop, you want `r3d_anim_play` |
...
| `r3d_anim_speed(h) -> f64` | the rate the base clip is actually being played at (1.0 = as authored) | -1.0 for a handle that is not a model, because 0.0 is a legitimate rate - a body held on one frame - and a sentinel that collides with a real answer cannot be acted on. Completes `r3d_anim_clip` (which) and `r3d_anim_time` (how far), so the three describe one layer. Added because a game that retimes a clip to meet a tick budget could not check the result against anything but the number it had just calculated: both operands of the assertion came from the code under test, so it held whatever that code did |
...
| `r3d_anim_blend(h, a, b, weight, speed, fade, looping)` | hold a SUSTAINED blend of two clips on the base layer | Not a crossfade: the pair is the pose, held at `weight`, and called every frame to track a continuous value like movement speed. Each clip keeps its own clock so two loops of different lengths each run at their own cadence, and changing either clip crossfades out of the composed pose rather than out of one half of it. `r3d_anim_blend_space` is the same thing with a direction axis |
| `r3d_anim_blend_space(h, a0, a1, b0, b1, dir, weight, speed, fade, looping)` | a 2x2 locomotion blend space: two tiers, two directions in each | `a0`/`a1` are the lower tier and `b0`/`b1` the upper - idle and walk, or walk and run - and `dir` mixes within BOTH at once, because a stick pointing half-right points half-right whether you are walking or running. It exists because locomotion was QUANTISED: six directional clips chosen by thresholding the stick, so walking thirty degrees right showed a pure forward stride or a pure sideways one and never the mix. Crossfading between them is not the fix - a fade smooths the join between two poses that are both wrong for where the player is going. `r3d_anim_blend` is this with each tier collapsed to one clip, so a caller with no direction axis uses the same mechanism. The clips in a tier share a duration exactly, so one clock per tier holds its pair in phase with nothing to synchronise |
